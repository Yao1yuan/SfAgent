...
### 1.6 Input Image Acquisition Strategy (CRITICAL)
- **THE ANTI-PATTERN**: NEVER attempt to directly access an `Image Source` tool (e.g., `CogInputImageTool`) from within a script via direct type casting.
- **REASON**: The script's compiler sandbox does not have a reference to the assembly defining `CogInputImageTool`. All direct type casting attempts will fail at compile time. This has been **experimentally verified**.
- **THE CORRECT PATTERN**: The script MUST obtain its input image from the **first standard processing tool** in the chain that receives the image (e.g., `CogBlobTool`, `CogPMAlignTool`). The VPP's data binding engine guarantees the image is already present in that tool's `.InputImage` property before the script executes. This is the simplest, fastest, and only reliable method.
